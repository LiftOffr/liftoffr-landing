...
**I know because I shipped exactly that bug.** Until 20 August 2026 my own code averaged missing
readings in as zeros. On 19 August that understated the published Score by 1.8 points — it read
34.5 when it should have read 36.3. Anyone who pulled the data and recomputed it properly would
have got a different number from mine, and they would have been right. It is fixed, the
correction is dated on liftoffr.com/score, and the 64-signal record was never affected because
that log was generated by code that handled it correctly.

That is the practical case for publishing a method rather than a conclusion: a published method
can be checked by the people using it, and this one was wrong for long enough that it should
have been.
...
liftoffr.com/receipts and recompute that date. Same rule about the divisor applies, and it bites
harder historically — Pi Cycle alone has no reading on several hundred of the dates in that log.
A recompute that always divides by 1.00 will disagree with a few hundred rows, and it will be
