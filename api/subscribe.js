...
import { validUnsubscribeToken, unsubscribeFromAudiences, unsubscribeHeaders } from "./_email-preferences.js";
...
export default async function handler(req, res) {
  // Signed GET footer links and RFC 8058 one-click POST share the same checks.
  const uq = new URL(req.url, "http://localhost").searchParams;
  if (uq.get("u") === "1") {
    const isPost = req.method === "POST";
    const reply = (status, message) => {
      if (isPost) return res.status(status).json({ ok: status === 200, message });
      res.setHeader("Content-Type", "text/html; charset=utf-8");
      return res.status(status).send(unsubPage(message));
    };
    if (!isPost && req.method !== "GET") {
      res.setHeader("Allow", "GET, POST");
      return reply(405, "Method not allowed");
    }
    if (!process.env.CRON_SECRET) return reply(503, "Email preferences are temporarily unavailable. Please try again shortly.");
    const email = (uq.get("e") || "").trim().toLowerCase();
    if (!EMAIL_RE.test(email) || !validUnsubscribeToken(email, uq.get("t"), process.env.CRON_SECRET)) {
      return reply(400, "That unsubscribe link is incomplete or invalid. Use the link in your latest email, or reply and I'll remove you manually.");
    }
    try {
      await unsubscribeFromAudiences(email, process.env);
      return reply(200, "You're unsubscribed from LiftOffr emails. Changed your mind? Reply to any past email.");
    } catch (error) {
      console.error("subscribe: unsubscribe update failed", error.message);
      return reply(502, "We couldn't finish updating your email preferences. Please try this link again shortly, or reply to an email for help.");
    }
  }
