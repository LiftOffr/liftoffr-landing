...
const config={WHOP_API_KEY:'test-whop',PLAN_CREDIT_SECRET:'fixture-stable-credit-secret-at-least-32chars',RESEND_API_KEY:'test-resend',WHOP_WEBHOOK_SECRET:'test-signature-secret',GA4_MEASUREMENT_ID:'G-TEST',GA4_API_SECRET:'test-ga',RESEND_PLAN_AUDIENCE_ID:'test-audience'};
...
test('verified payment forwards actual subtotal once with payment id',async()=>fixture({},async({calls})=>{
 ...
 const p=ga[0].body.events[0].params;assert.equal(p.value,27.62);assert.equal(p.transaction_id,'pay_fixture');assert.equal(p.attribution_status,'browser_session_unlinked');
}));
