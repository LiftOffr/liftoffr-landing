...
r=await render({components:{Woobull:{value:25,asOf:null}}});assert.equal(r.main.textContent,'Unavailable');assert.equal(r.stamp.textContent,'No current component reading');
r=await render({},true);assert.equal(r.main.textContent,'Unavailable');assert.match(r.status.textContent,/Could not verify/);
console.log('Indicator live checks passed: missing component, invalid value, component date, missing date, network failure');
